...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		{
			float highestInfluance = 0.0f;
			for (auto connection : GetNodeConnections(node->GetIndex()))
			{
				float neighborsInfluence = GetNode(connection->GetTo())->GetInfluence() * expf(-connection->GetCost() * m_Decay);
				if (abs(highestInfluance) > abs(neighborsInfluence))
					continue;

				highestInfluance = neighborsInfluence;
			}
